...
let menu (ctx: SiteContents) (page: string) =
    ...
    let group =
        content
        |> Seq.tryFind (fun n -> n.title = page)
        |> Option.map (fun n -> n.category)

    let explenations =
        content
        |> Seq.filter (fun n ->
            n.category = Contentloader.Explanation
            && not n.hide_menu)
    ...
    let tutorials =
        content
        |> Seq.filter (fun n ->
            n.category = Contentloader.Tutorial
            && not n.hide_menu)
        |> Seq.sortBy (fun n -> n.menu_order)
    ...
    let howtos =
        content
        |> Seq.filter (fun n ->
            n.category = Contentloader.HowTo
            && not n.hide_menu)
        |> Seq.sortBy (fun n -> n.menu_order)
    ...
    let menuHeader =
        [ if hasExplenations then
              li [ Id "menu-explanations"
                   if group = Some Contentloader.Explanation then
                       Class "dd-item menu-group-link menu-group-link-active"
                   else
                       Class "dd-item menu-group-link" ] [
                  a [] [ !! "Explanation" ]
              ]
          if hasTutorials then
              li [ Id "menu-tutorials"
                   if group = Some Contentloader.Tutorial then
                       Class "dd-item menu-group-link menu-group-link-active"
                   else
                       Class "dd-item menu-group-link" ] [
                  a [] [ !! "Tutorials" ]
          ...
          if hasHowTos then
              li [ Id "menu-howtos"
                   if group = Some Contentloader.HowTo then
                       Class "dd-item menu-group-link menu-group-link-active"
                   else
                       Class "dd-item menu-group-link" ] [
                  a [] [ !! "How-To Guides" ]
    ...
    let renderRefs =
        ul [ Id "submenu-refs"
            ...
            for r in all ->
                li [] [
                    a [ Href(rootUrl + "/reference/" + r.Label + "/index.html")
    ...
    nav [ Id "sidebar" ] [
        div [ Id "header-wrapper" ] [
            ...
            div [ Class "searchbox" ] [
                label [ Custom("for", "search-by") ] [
                    i [ Class "fas fa-search" ] []
                ]
                input [ Custom("data-search-input", "")
                        Id "search-by"
                        Type "search"
                        Placeholder "Search..." ]
                span [ Custom("data-search-clear", "") ] [
