...
let generateType ctx (page: ApiPageInfo<Type>) =
    ...
    let body =
        div [ Class "api-page" ] [
            h2 [] [ !!t.Name ]
            b [] [ !! "Namespace: " ]
            a [ Href(sprintf "%s.html" page.NamespaceUrlName) ] [
                !!page.NamespaceName
            ]
            br []
            b [] [ !! "Parent: " ]
            a [ Href(sprintf "%s.html" page.ParentUrlName) ] [
                !!page.ParentName
            ]
            ...
            table [] [
                ...
                if not t.Constructors.IsEmpty then
                    tr [] [
                        td [ ColSpan 3. ] [
                            b [] [ !! "Constructors" ]
                        ]
                ...
                if not t.InstanceMembers.IsEmpty then
                    tr [] [
                        td [ ColSpan 3. ] [
                            b [] [ !! "Instance Members" ]
                        ]
                ...
                if not t.RecordFields.IsEmpty then
                    tr [] [
                        td [ ColSpan 3. ] [
                            b [] [ !! "Record Fields" ]
                        ]
                ...
                if not t.StaticMembers.IsEmpty then
                    tr [] [
                        td [ ColSpan 3. ] [
                            b [] [ !! "Static Members" ]
                        ]
                ...
                if not t.StaticParameters.IsEmpty then
                    tr [] [
                        td [ ColSpan 3. ] [
                            b [] [ !! "Static Parameters" ]
                        ]
                ...
                if not t.UnionCases.IsEmpty then
                    tr [] [
                        td [ ColSpan 3. ] [
                            b [] [ !! "Union Cases" ]
                        ]
...
let generate' (ctx: SiteContents) =
    ...
    | Some all ->
        ...
        |> List.collect (fun n ->
            ...
            let modules = n.Modules |> Seq.map (generateModule ctx)

            let types = n.Types |> Seq.map (generateType ctx)
