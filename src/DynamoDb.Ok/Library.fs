...
module Write =
  ...
  module BuildAttr =
    ...
    let optional name attr = function
      | Some s -> [ Attr (name, attr s) ]
      | None -> []

    let setString name = function
      | h::t -> [ Attr (name, NonEmptyList (h, t) |> SetString) ]
      | _ -> []

    let setDecimal name = function
      | h::t -> [ Attr (name, NonEmptyList (h, t) |> SetDecimal) ]
      | _ -> []

    let string name s =
      if String.IsNullOrEmpty s then []
      else [ Attr (name, ScalarString s) ]

  module BuildAttrValue =

    let optionToNull attr = function
      | Some s -> attr s
      | None -> ScalarNull
...
module Read =
  ...
  type AttrReader<'a> =
    AttrReader of (Map<string, A> -> 'a)

  module AttrReader =

    let run (AttrReader f) a =
      f a
  ...
  let attrReaderResult =
    new AttrReaderResultBuilder ()
  ...
  let private traverseResult f list =
    let folder head tail =
      f head |> Result.bind (fun h ->
        tail |> Result.bind (fun t -> Ok (h::t)))
    List.foldBack folder list (Ok [])

  let private ifSome f = function
    | Some x -> f x |> Result.map Some
    | None -> Ok None

  let private required key = function
    | Some x -> Ok x
  ...
  module Query =
    ...
    let private boolOperatorToExpression = function
      | And -> "AND"
      | Or  -> "OR"
    ...
    let private getAttribute attributes key value format =
      let attributeName, attributes =
        getAttributeName attributes value
      sprintf format key attributeName, attributes

    let private keyConditionToString attributes = function
  ...
  let private queryScanIndexDirection forward (client:AmazonDynamoDBClient) tableName limit reader kce =
    let expression, attrs =
      Query.buildKeyConditionExpression kce []
    let buildAttributes =
  ...
  let query client  =
    queryScanIndexDirection true client

  let queryReverse client =
    queryScanIndexDirection false client
  ...
  module Parse =
    ...
    let private fromByRef e = function
      | true, x -> Ok x
...
module Example =
  ...
  type Outty =
    { F: String; G: DateTime; I: Inny; H: Inny list; J: Decimal list; K: String list; L : DateTime list option }

  let buildOutty f g i h j k l =
    { F = f; G = g; I = i; H = h; J = j; K = k; L = l }
